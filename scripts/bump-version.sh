...
#
# --push: also commit, tag v<version> and push both to origin, which runs the Release and Deploy
...
plugin='bestony-ai-provider-for-stepfun.php'
config='src/Util/StepfunConfig.php'
...
for file in "$plugin" readme.txt "$config"; do
    grep -q "$next" "$file" || {
...
Next:
  ...
  2. scripts/bump-version.sh $part --push     # commits, tags v$next and pushes
EOF
fi
